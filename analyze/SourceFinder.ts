...
//     if (input.exclude) await decode(input.exclude, (str) => dict.delete(str));

//     return [...dict];
//   }

//   async function decode(input: string[], closure: (location: string) => void): Promise<void> {
//     for (const pattern of input)
//       for (const location of await _Glob(path.resolve(pattern))) {
//         const stats: fs.Stats = await fs.promises.stat(location);
//         if (stats.isDirectory() === true) await iterate(closure, location);
//         else if (stats.isFile() && _Is_ts_file(location)) closure(location);
//       }
//   }

//   async function iterate(closure: (location: string) => void, location: string): Promise<void> {
