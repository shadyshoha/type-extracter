...
//     if (!(creator instanceof Function && creator.constructor instanceof Function)) return null;
//     // MUST HAVE THOSE MATADATA
//     else if (
//       ArrayUtil.has(Reflect.getMetadataKeys(creator), "path", "host", "scope:options") === false
//     )
//       return null;
...
//     if (parent.prototype !== undefined) tuple.push(..._Get_prototype_entries(parent));
...
//     else if (ArrayUtil.has(Reflect.getMetadataKeys(proto), "path", "method") === false) return null;
...
//         Reflect.getMetadata("__interceptors__", proto)[0]?.constructor?.name ===
//           "EncryptedRouteInterceptor",
//     };
...
//       Pather.join(controller.path, meta.path).split("\\").join("/"),
//       ":",
//       "/"
//     ).sort();
...
//     if (symbol.indexOf("__custom") !== -1) return _Analyze_custom_parameter(param);
